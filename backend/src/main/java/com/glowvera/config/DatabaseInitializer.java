...
public class DatabaseInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Service s1 = getOrCreateService(hairCare, "Unisex Haircut & Styling", new BigDecimal("35.00"), 45);
        Service s2 = getOrCreateService(hairCare, "Luxury Hair Spa & Deep Conditioning", new BigDecimal("50.00"), 60);
        Service s3 = getOrCreateService(hairCare, "Professional Hair Coloring", new BigDecimal("90.00"), 120);
        Service s4 = getOrCreateService(hairCare, "Hair Blowout & Styling", new BigDecimal("25.00"), 30);

        Service s5 = getOrCreateService(beardGrooming, "Classic Beard Trim & Razor Styling", new BigDecimal("20.00"), 30);
        Service s6 = getOrCreateService(beardGrooming, "Hot Towel Royal Shave", new BigDecimal("30.00"), 45);

        Service s7 = getOrCreateService(spaMassage, "Swedish Deep Tissue Massage", new BigDecimal("80.00"), 60);
        Service s8 = getOrCreateService(spaMassage, "Hot Stone Relaxing Therapy", new BigDecimal("95.00"), 90);

        Service s9 = getOrCreateService(skinFacial, "Charcoal Deep Cleansing Facial", new BigDecimal("45.00"), 45);
        Service s10 = getOrCreateService(skinFacial, "Luxury Anti-Aging Facial", new BigDecimal("75.00"), 60);
